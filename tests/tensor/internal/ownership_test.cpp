...
namespace minitensor::test
{
    void run_ownership_test()
    {
        ...
        {
            NodeRef producer = std::make_shared<Node>(std::make_unique<IdentitySpecPrimitive>(),
                                                      std::vector<ValueRef>{});
            const std::weak_ptr<const Node> weak_producer{producer};

            {
                const auto output = std::make_shared<Value>(spec, producer);
                producer.reset();
        ...
        {
            ValueRef input = std::make_shared<Value>(spec);
            ...
            NodeRef producer = std::make_shared<Node>(std::move(primitive),
        ...
        ValueRef leaf = std::make_shared<Value>(spec);
