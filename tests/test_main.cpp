...
void test_forward_operations() {
    ...
    expect_close(tensor.sum().item(), 21.0F, "sum all elements");
    expect_values(tensor.sum(0), {5, 7, 9}, "sum along dimension zero");
    expect_values(tensor.sum(1, true), {6, 15}, "sum keepdim values");
    expect(tensor.sum(1, true).shape() == Shape{2, 1}, "sum keepdim shape");
...
void test_elementwise_autograd() {
    ...
    const auto branch_loss = (branched * branched + branched).sum();
    ...
    (numerator / denominator + numerator - denominator).sum().backward();
...
void test_view_and_reduction_autograd() {
    auto input = matrix_2x3(true);
    input.slice(1, 0, 3, 2).sum().backward();
    ...
    const auto loss = (transposed_input.transpose(0, 1).reshape({2, 3}) * weights).sum();
    loss.backward();
    expect_values(
    ...
    const auto reduction = reduction_input.sum(0);
...
void test_matmul_and_activation_autograd() {
    ...
    minitensor::matmul(lhs, rhs).sum().backward();
...
void test_validation() {
    ...
    expect_throws<std::out_of_range>(
        [] { static_cast<void>(Tensor::zeros({2, 3}).sum(-1)); },
