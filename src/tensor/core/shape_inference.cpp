...
namespace minitensor::detail
{
	namespace
	{
		std::vector<Extent> broadcast_extents(Shape::size_type output_rank, const Shape& lhs, const Shape& rhs, Shape::size_type lhs_effective_rank, Shape::size_type rhs_effective_rank)
		{
	...
	{
		const Shape::size_type output_rank = std::max(lhs.rank(), rhs.rank());
	...
	Shape matmul_output_shape(const Shape& lhs, const Shape& rhs)
	{
		...
		const Shape::size_type lhs_batch_rank = lhs_is_vector ? 0 : lhs.rank() - 2;
		const Shape::size_type rhs_batch_rank = rhs_is_vector ? 0 : rhs.rank() - 2;
		const Shape::size_type output_batch_rank = std::max(lhs_batch_rank, rhs_batch_rank);
