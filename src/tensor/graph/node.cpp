...
{
    Node::Node(NodeId id, PrimitiveRef primitive, std::vector<ValueRef> inputs) : id_(id), primitive_(std::move(primitive)), inputs_(std::move(inputs))
    {
        if (!primitive_)
        {
    ...
    const PrimitiveRef &Node::primitive_ref() const noexcept
    {
        return primitive_;
    }
