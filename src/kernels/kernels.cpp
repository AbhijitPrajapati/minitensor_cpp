...
float apply_binary(const float lhs, const float rhs, const BinaryKernel operation) {
    switch (operation) {
    case BinaryKernel::add: return lhs + rhs;
    case BinaryKernel::subtract: return lhs - rhs;
    case BinaryKernel::multiply: return lhs * rhs;
    case BinaryKernel::divide: return lhs / rhs;
    }
...
float apply_unary(const float value, const UnaryKernel operation) {
    switch (operation) {
    case UnaryKernel::negate: return -value;
    case UnaryKernel::relu: return std::max(0.0F, value);
    ...
    case UnaryKernel::tanh: return std::tanh(value);
    }
